...
from ..helpers import serialize_common
from .common import (
    Author,
    BoolValue,
    Category,
...
class Signal:
    ...
    def __str__(self) -> str:
        ret = (
            ...
            + " {} {} {} {}\n".format(
                self.required, self.negated, self.clock, self.forced_net
            )
            + ")"
        )
        return ret
...
class PinSignalMap:
    def __init__(
        self, pin_uuid: str, signal_uuid: SignalUUID, text_designator: TextDesignator
    ):
    ...
    def __str__(self) -> str:
        return "(pin {} {} {})".format(
            self.pin_uuid, self.signal_uuid, self.text_designator
        )
...
class Gate:
    ...
    def __str__(self) -> str:
        ret = (
            ...
            + " {} {} {} {}\n".format(
                self.position, self.rotation, self.required, self.suffix
            )
        )
        pin_lines = []
...
class Variant:
    def __init__(
        self, uuid: str, norm: Norm, name: Name, description: Description, gate: Gate
    ):
        self.uuid = uuid
        self.norm = norm
        self.name = name
